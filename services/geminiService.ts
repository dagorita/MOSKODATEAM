...
export const generateArcadeResponse = async (userMessage: string, history: { role: string; text: string }[] = []): Promise<string> => {
  try {
    const ai = getClient();
    
    // Convert history to format expected by the API if needed, 
    // but for simple single-turn or short context, we can just use generateContent with system instruction.
    // For better chat context, we construct a prompt chain.
    ...
    return "SYSTEM ERROR: CONNECTION LOST. PLEASE CHECK YOUR WIRING.";
  }
};